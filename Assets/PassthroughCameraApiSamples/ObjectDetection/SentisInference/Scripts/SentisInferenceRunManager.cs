        ...
        private void GetInferencesResults()
        {
            ...
            switch (m_download_state)
            {
                ...
                case 2:
                    ...
                    break;
                case 3:
                    // Log the raw detection results so they are visible in the Unity Console / device logs.
                    // try
                    // {
                    //     var labels = m_labelsAsset != null ? m_labelsAsset.text.Split('\n') : null;
                    //     var boxesFound = m_output.shape[0];
                    //     Debug.Log($"Sentis: {m_output} boxes found");
                    //     for (int i = 0; i < boxesFound; ++i)
                    //     {
                    //         // The output tensor layout used by the UI: [centerX, centerY, width, height, ...]
                    //         var cx = m_output[i, 0];
                    //         var cy = m_output[i, 1];
                    //         var w = m_output[i, 2];
                    //         var h = m_output[i, 3];
                    //         var labelId = (m_labelIDs != null && m_labelIDs.shape[0] > i) ? m_labelIDs[i] : -1;
                    //         var labelName = (labels != null && labelId >= 0 && labelId < labels.Length) ? labels[labelId] : labelId.ToString();
                    //         Debug.Log($"Sentis detection {i}: class={labelName} (id={labelId}) center=({cx:0.##},{cy:0.##}) size=({w:0.##}x{h:0.##})");
                    //     }
                    // }
                    // catch (Exception e)
                    // {
                    //     Debug.LogError($"Error logging Sentis detections: {e.Message}");
                    // }
